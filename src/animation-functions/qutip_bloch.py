__all__ = ['Bloch']
## qBraid UPDATED BLOCH SPHERE CLASS
import os
...
sigmax = operators.sigmax
sigmay = operators.sigmay
sigmaz = operators.sigmaz
import numpy as np
...
class Bloch:
    ...
    def add_states(self, state, kind='vector', color='r'):
        ...
        for st in state:
            ...
            if kind == 'vector':
                self.add_vectors(vec, color)
            elif kind == 'point':
                self.add_points(vec)

    # def add_extra_lines(self, line, color='g'):
    #     if isinstance(vectors[0], (list, ndarray)):
    #         for vec in vectors:
    #             self.extra_lines.append(vec)
    #             if color:
    #                 self.vector_color_array.append(color)
    #     else:
    #         self.extra_lines.append(vectors)
    #         if color:
    #                 self.vector_color_array.append(color)
                    
    def add_vectors(self, vectors, color='g'):
    ...
    def render(self):
        ...
        self.plot_vectors()
        self.plot_front()
        # self.axes.plot([1,1], [0,0], [-1,1],
        #                        lw=self.vector_width, color='#800080')
        ...
        for k in range(len(self.vectors)):
            ...
            if self.vector_style == '':
                self.axes.plot(xs3d, ys3d, zs3d,
                               zs=0, zdir='z', label='Z',
                               lw=self.vector_width, color=self.vector_color_array[k])
                ...
                a = Arrow3D(xs3d, ys3d, zs3d,
                            ...
                            color=self.vector_color_array[k])
